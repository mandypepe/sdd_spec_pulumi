"""
🇪🇸 Componente Bóveda para GCP con soporte multi-zona y seguridad perimetral.
🇺🇸 Vault Component for GCP with multi-zone and perimeter security support.
"""
...
class GcpVaultComponent(VaultComponentResource):
    """
    🇪🇸 Componente para Bóveda de Secretos en GCP con:
    - Aislamiento multi-zona (US1)
    - Cifrado en reposo y rotación (US2)
    - Federación de identidad (US3)

    🇺🇸 Component for Secrets Vault on GCP with:
    ...
    def configure_identity_federation(self, trust_provider_url: str, allowed_namespaces: List[str]):
        ...
        pulumi.info(f"Configuring GCP Vault identity federation")
        # Placeholder for Workload Identity Federation
...
VaultProviderFactory.register("gcp", GcpVaultComponent)
